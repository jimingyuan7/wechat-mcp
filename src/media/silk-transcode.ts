...
export async function silkToWav(silkBuf: Buffer): Promise<Buffer | null> {
  try {
    // Optional dependency: voice transcoding only works when `silk-wasm` is
    // installed. Indirecting the specifier keeps it out of compile-time module
    // resolution so the package remains optional.
    const silkModule = "silk-wasm";
