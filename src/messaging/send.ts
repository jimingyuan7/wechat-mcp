...
/** Minimal reply payload (replaces the OpenClaw SDK `ReplyPayload`). */
export interface ReplyPayload {
  text?: string;
}

function generateClientId(): string {
  return generateId("wechat-mcp");
}
...
/** Build a SendMessageReq from a reply payload (text only). */
...
/**
 * Send one MessageItem (optionally preceded by a text caption) downstream.
