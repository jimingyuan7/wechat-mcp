...
export async function displayQRCode(qrcodeUrl: string): Promise<void> {
  try {
    ...
    // Render to a string and write to STDERR (not STDOUT) so this is safe to
    // call while an MCP stdio server owns STDOUT for JSON-RPC.
    ...
    process.stderr.write(`若二维码未能显示或无法使用，你可以访问以下链接以继续：\n`);
    process.stderr.write(`${qrcodeUrl}\n`);
  } catch {
    process.stderr.write(`若二维码未能显示或无法使用，你可以访问以下链接以继续：\n`);
    process.stderr.write(`${qrcodeUrl}\n`);
  }
...
async function refreshQRCode(
  ...
  try {
    ...
    process.stderr.write(`🔄 二维码已更新，请重新扫描。\n\n`);
